    ...
    <div className="fixed inset-0 z-[100] flex justify-center items-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in-0">
      <div className="bg-white p-6 rounded-2xl shadow-xl max-w-sm w-full border border-slate-200 animate-in fade-in-0 zoom-in-95 duration-300">
        <div className="flex justify-between items-center pb-3 border-b border-slate-200">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition-colors rounded-full p-1"><X size={20} /></button>
        </div>
...
// --- Komponen Utama ---
const LacakPengembaliandariApotek = () => {
  ...
  const fetchTrackingData = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/pbf/pesanan-apotek/lacak-pengembalian/${id}`, {
      ...
      setTrackingData({
        ...
        noSuratJalanBerangkat: data.nomor_surat_jalan, // SJ Apotek
        noSuratJalanPulang: data.nomor_surat_jalan_pengembalian || `SJ-RET-PBF-${String(data.id).padStart(3,'0')}`,
        pengirim: data.nama_apotek, 
        ...
        waktuPenerimaanPbf: data.tanggal_penerimaan_pbf, 
        estimasiSampai: estimasiSampai,
        status: data.status,
        buktiFotoPengembalian: data.bukti_foto, // Bukti dari Apotek (awal)
        buktiPenerimaanPbf: data.bukti_foto_pengembalian // Bukti dari PBF (akhir)
      });
  ...
  const handleConfirmReceipt = async () => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/pbf/pesanan-apotek/lacak-pengembalian/${id}/konfirmasi`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDownloadPDF = () => {
    ...
    const opt = {
        margin:       [10, 5, 10, 5],
        filename:     `lacak_pengembalian_apotek_${trackingData.noSuratJalanPulang}.pdf`,
  ...
  if (!trackingData) return null; // Fallback
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50">
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div ref={contentRef} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden print:shadow-none print:border-0 print:p-4">
              <div className="bg-gradient-to-r from-emerald-500 to-teal-600 px-6 py-5 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                 <div>
                   <h1 className="text-2xl font-bold text-white">Lacak Pengembalian dari Apotek</h1>
              ...
              <div className="p-8 border-b border-slate-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-1">
                        <span className="text-sm font-medium text-slate-500">Pengirim (Apotek)</span>
                        <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                          <span className="font-semibold text-slate-900 text-base">{trackingData.pengirim}</span>
                        </div>
                    </div>
                    <div className="space-y-1">
                        <span className="text-sm font-medium text-slate-500">Tujuan (PBF)</span>
                        <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
              ...
              <div className="p-8 py-12">
                ...
                <div className="flex items-start justify-center gap-0">
                  ...
                  <StatusStep
                    ...
                    timestamp={isSelesai ? formatTimestamp(trackingData.waktuPenerimaanPbf) : `Estimasi: ${formatDate(trackingData.estimasiSampai)}`}
                    isCompleted={isSelesai}
                    isCurrent={currentStatusStep === 'Selesai'}
                  >
                    {isSelesai && trackingData.buktiPenerimaanPbf && (
                ...
                <div className="flex-1">
                    ...
                    {trackingData.status === 'Pengembalian Disetujui' && (
                        <div className="bg-blue-50 border border-blue-200 text-blue-800 rounded-lg p-3 text-sm flex items-center gap-3">
                          <Info size={18} className="flex-shrink-0"/>
                          <span>Menunggu Apotek mengirimkan barang retur. Status akan diperbarui.</span>
                        </div>
                    )}
      ...
      <ProofModal
        ...
        imageUrl={trackingData.buktiPenerimaanPbf}
      />
       <style>
    {`
      @media print {
...
export default LacakPengembaliandariApotek;