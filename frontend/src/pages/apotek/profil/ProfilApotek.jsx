        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          {isEditing ? (
            <>
              ...
              <EditItem label="Nama Resmi Apotek" name="nama_resmi" value={formData.nama_resmi} onChange={handleInputChange} icon={Building} required />
              <EditItem label="Kontak Telepon" name="kontak_telepon" value={formData.kontak_telepon} onChange={handleInputChange} icon={Phone} />
              <EditItem label="Nomor Izin (SIA)" name="nomor_izin" value={formData.nomor_izin} onChange={handleInputChange} icon={FileText} required />
              <EditItem label="Nama Apoteker PJ" name="nama_apoteker" value={formData.nama_apoteker} onChange={handleInputChange} icon={User} required />
              <EditItem label="Nomor SIPA" name="nomor_sipa" value={formData.nomor_sipa} onChange={handleInputChange} icon={FileText} required />
...
const SecuritySettings = () => {
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      // Panggil API Apotek
      const response = await axios.put('http://localhost:5000/api/apotek/change-password', {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${isCollapsed ? 'ml-16' : 'ml-64'}`}>
        <NavbarApotek onLogout={handleLogout} username={username} onToggleSidebar={() => setIsCollapsed(!isCollapsed)} />
        ...
        <main className="flex-1 overflow-auto pt-[72px] px-12 py-8">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-8">
              ...
              <p className="text-slate-600 text-lg mt-1">Kelola informasi akun dan keamanan Anda.</p>
            </div>
