// frontend/src/pages/pbf/pesanobat/LihatRiwayatPesanan.jsx
...
// --- Komponen Modal (Sama seperti versi Produsen) ---
...
// --- Komponen StatusStep (Sama seperti versi Produsen) ---
...
const LihatRiwayatPesanan = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:5000/api/pbf/pesanan/riwayat/${assetId}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!response.ok) {
          const errData = await response.json();
          throw new Error(errData.message || 'Gagal mengambil data riwayat');
        }
  ...
  if (!riwayatData || !riwayatData.onChain || !riwayatData.offChain) return <div className="p-6 text-center text-gray-500">Data riwayat tidak ditemukan.</div>;
