...
const PerluDikirimApotek = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!token) { navigate('/login/apotek'); return; }
  ...
  const filteredData = useMemo(() => {
    return pesananData
      .filter(item => item.status === 'Perlu Dikirim') // <-- FILTER HALAMAN INI
      .filter(item =>
        // --- PERBAIKAN SEARCH ---
            ...
            <div className="mb-10 relative">
              ...
              <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center gap-3">
                  <div className="flex items-center justify-center w-12 h-12 bg-gradient-to-br from-orange-500 to-amber-600 rounded-xl shadow-lg">
                    <Package className="text-white" size={24} />
                  </div>
                  <div>
                    <h1 className="text-4xl font-bold bg-gradient-to-r from-slate-900 via-orange-900 to-amber-900 bg-clip-text text-transparent">
                      Pesanan Perlu Dikirim
                    </h1>
                    <p className="text-slate-600 text-lg mt-1">Pesanan yang telah dikonfirmasi PBF.</p>
                  </div>
                </div>
                 <button 
              ...
              {isLoading && pesananData.length > 0 ? (
                ...
                <div className="text-center py-12 bg-white rounded-lg border border-dashed border-slate-300">
                  ...
                  <p className="text-slate-500 font-medium">
                    {searchTerm ? "Tidak ada pesanan yang sesuai." : "Tidak ada pesanan yang perlu dikirim."}
                  </p>
                </div>
              )}
            </div>
...
export default PerluDikirimApotek;