...
const ProdusenDashboard = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [riwayatResponse, pesananResponse, produksiResponse] = await Promise.all([
          ...
          fetch('http://localhost:5000/api/produsen/pesanan-masuk', {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ...
        if (riwayatResponse.ok) {
          ...
          if (!riwayatResult.success)
            console.warn('Respons riwayat tidak berhasil:', riwayatResult.message);
        } else {
        ...
        if (pesananResponse.ok) {
          ...
          if (!pesananResult.success)
            console.warn('Respons pesanan tidak berhasil:', pesananResult.message);
        } else {
        ...
        if (produksiResponse.ok) {
          ...
          if (!produksiResult.success)
            console.warn('Respons produksi tidak berhasil:', produksiResult.message);
        } else {
          console.warn(
            'Gagal mengambil data produksi:',
            produksiResponse.status,
        ...
        const mappedRiwayat = (riwayatResult.data || [])
          ...
          .map((item) => ({
            ...
            status:
              item.status_blockchain === 'DITERIMA_PBF'
                ? 'Diterima'
                : item.status_blockchain === 'DIKIRIM_KE_PBF'
                  ? 'Dikirim'
                  : item.status === 'Selesai'
                    ? 'Diterima'
                    : 'Dikirim',
        ...
        const stokTersedia = (produksiResult.data || []).reduce(
          (acc, cur) => acc + (cur.jumlah || 0),
          0
        );
        const totalProduksi = (produksiResult.data || []).length;
        const produksiSelesai = (produksiResult.data || []).filter(
          (p) => p.status === 'Selesai' || p.status === 'Tercatat di Blockchain'
        ).length;
        ...
        setStats({
          ...
          efisiensiProduksi:
            totalProduksi > 0
              ? parseFloat(((produksiSelesai / totalProduksi) * 100).toFixed(1))
              : 0,
        });
      } catch (error) {
  ...
  const StatCard = ({ icon, value, label, unit, isCurrency = false, trend, color = 'emerald' }) => {
    ...
    return (
      <div className="group relative bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:shadow-xl hover:border-slate-300 transition-all duration-300 overflow-hidden">
        <div
          className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${selectedColor.bg} opacity-5 rounded-full -mr-16 -mt-16 group-hover:scale-150 transition-transform duration-500`}
        ></div>
  ...
  const getActivityIcon = (type) => {
    switch (type) {
      case 'Pesanan':
        return <ShoppingCart size={18} className="text-purple-600" />;
      case 'Pengiriman':
        return <Truck size={18} className="text-blue-600" />;
      case 'Produksi':
        return <Factory size={18} className="text-emerald-600" />;
      default:
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-xl border border-gray-100">
              <div className="flex justify-between items-center mb-6">
                ...
                <button className="inline-flex items-center text-emerald-600 hover:text-emerald-700 font-medium transition-colors group text-sm">
                  Lihat Semua
                  <ArrowRight
                    size={16}
