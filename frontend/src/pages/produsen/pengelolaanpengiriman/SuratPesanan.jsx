...
const SuratPesanan = () => {
  ...
  const handlePrint = () => {
    ...
    setTimeout(() => { // Beri waktu untuk memuat gambar/gaya
        printWindow.print();
        printWindow.close();
    }, 1000);
  };
              ...
              <section className="grid grid-cols-2 gap-8 mb-10 text-sm print:text-xs print:gap-4 print:mb-6">
                  <div className="space-y-1">
                   ...
                   <div className="space-y-1 text-right">
                       <h3 className="font-bold text-slate-800 mb-2 uppercase text-xs tracking-wider border-b pb-1">Kepada Yth. (Produsen)</h3>
                       <p className="font-semibold text-slate-700">{info.nama_produsen || 'Nama Produsen Anda'}</p>
                  ...
                  <table className="w-full text-sm print:text-xs">
                    <thead className="bg-slate-100 text-slate-700 print:bg-slate-200">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-left font-semibold border-b border-slate-200 print:border-slate-400">Bentuk Sediaan</th>
                    ...
                    <tbody className="divide-y divide-slate-100 print:divide-slate-300">
                      {detail.map((item, index) => (
                        <tr key={item.id || index}>
                          ...
                          <td className="px-3 py-2 border-r border-slate-200 print:border-slate-400">{item.bentuk_sediaan || '-'}</td>
                          <td className="px-3 py-2 text-center border-r border-slate-200 print:border-slate-400">{item.jumlah_pesanan.toLocaleString('id-ID')} Box</td>
                          <td className="px-3 py-2 text-right border-r border-slate-200 print:border-slate-400">{(Number(item.harga_per_unit) || 0).toLocaleString('id-ID')}</td>
                          <td className="px-3 py-2 text-right">{(Number(item.total_harga) || 0).toLocaleString('id-ID')}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot className="bg-slate-100 print:bg-slate-200 font-semibold text-slate-800">
                      <tr>
                        <td colSpan="5" className="px-3 py-2 text-right border-t-2 border-slate-300 print:border-slate-400">TOTAL HARGA KESELURUHAN</td>
