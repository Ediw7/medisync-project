...
// --- FORMAT TANGGAL AMAN & KONSISTEN (DIPINDAH KE ATAS) ---
...
// --- TIMELINE PRODUSEN → PBF + PENGEMBALIAN ---
const ProdusenToPbfTimeline = ({ onChain, offChain, onShowBukti }) => {
  ...
  if (isPengembalian) {
    return (
      <div className="flex items-start justify-center gap-0">
        ...
        <StatusStep
          icon={XCircle}
          label={offChain.status}
          isCompleted={true}
          isCurrent={true}
        >
          <p className="text-xs text-slate-500 mt-1">
            {formatDate(offChain.updated_at, true)}
          </p>
        </StatusStep>
      </div>
    );
  ...
  return (
    <div className="flex items-start justify-center gap-0">
      <StatusStep icon={Archive} label="Dipesan" timestamp={getTimestamp('Dipesan')} isCompleted={isDipesan} isCurrent={currentStatus === 'Dipesan'} />
      <div className="relative flex items-center h-14 w-24"><div className={`w-full h-1.5 rounded-full ${isDikirim ? 'bg-emerald-500' : 'bg-slate-300'}`} /></div>
      <StatusStep icon={Truck} label="Dikirim Produsen" timestamp={getTimestamp('Dikirim')} isCompleted={isDikirim} isCurrent={currentStatus === 'Dikirim'} />
      <div className="relative flex items-center h-14 w-24"><div className={`w-full h-1.5 rounded-full ${isDiterima ? 'bg-emerald-500' : 'bg-slate-300'}`} /></div>
      <StatusStep icon={Home} label="Diterima PBF" timestamp={getTimestamp('Diterima')} isCompleted={isDiterima} isCurrent={currentStatus === 'Diterima'}>
...
const LihatRiwayatPesanan = () => {
  ...
  const { onChain, offChain, detail_pesanan } = riwayatData;
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              <div className="bg-gradient-to-r from-emerald-500 to-teal-600 px-6 py-5 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                ...
                <div className={`px-4 py-2 rounded-full border-2 text-sm font-semibold flex items-center gap-2 bg-white ${
                  currentStatus === 'Selesai' || currentStatus === 'Pengembalian Selesai' ? 'text-emerald-700 border-emerald-200' :
              ...
              <div className="p-8 border-b border-slate-200">
                <h3 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
                  <FileText size={20} className="text-emerald-600" /> Rincian Barang
                </h3>
                <div className="overflow-x-auto border border-slate-200 rounded-lg">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {detail_pesanan && detail_pesanan.length > 0 ? (
      ...
      <BuktiPenerimaanModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} imageUrl={offChain.buktiPenerimaUrl} />
    </div>
  );
};
