...
const SuratPesananApotek = () => {
  ...
  const totalHargaKeseluruhan = detail.reduce(
    (acc, item) => acc + (Number(item.harga_satuan * item.jumlah) || 0),
    0
  );
                      ...
                      {detail.map((item, index) => (
                        <tr key={item.id || index}>
                          ...
                          <td className="px-3 py-2 text-right">
                            {' '}
                            {Number(item.harga_satuan * item.jumlah || 0).toLocaleString('id-ID', {
