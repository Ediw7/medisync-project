...
import { Search, ArrowUpDown, Filter, X, Loader2, Package, AlertTriangle, Calendar, Plus } from 'lucide-react';
...
const ManajemenProduksi = () => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Apakah Anda yakin ingin menghapus jadwal produksi ini? Ini tidak dapat dibatalkan.')) return;
              ...
              <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-lg mt-1">Kelola jadwal dan status produksi obat Anda.</p>
                  </div>
                </div>
              ...
              <div className="p-4 border-b border-slate-200">
                  ...
                  {showFilters && (
                      <div className="p-4 bg-slate-50 mt-4 rounded-lg grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end border border-slate-200">
                          <select name="status" value={filters.status} onChange={handleFilterChange} className="p-2 border border-slate-300 rounded-lg w-full text-sm"><option value="">Semua Status</option><option value="Terjadwal">Terjadwal</option><option value="Dalam Produksi">Dalam Produksi</option><option value="Selesai">Selesai</option></select>
                          <select name="month" value={filters.month} onChange={handleFilterChange} className="p-2 border border-slate-300 rounded-lg w-full text-sm"><option value="">Semua Bulan</option>{[...Array(12).keys()].map(i => <option key={i+1} value={i+1}>{new Date(0, i).toLocaleString('id-ID', { month: 'long' })}</option>)}</select>
                          <input type="number" name="year" value={filters.year} onChange={handleFilterChange} placeholder="Tahun" className="p-2 border border-slate-300 rounded-lg w-full text-sm" />
                          <input type="number" name="minJumlah" value={filters.minJumlah} onChange={handleFilterChange} placeholder="Jumlah Min" className="p-2 border border-slate-300 rounded-lg w-full text-sm" />
                          <input type="number" name="maxJumlah" value={filters.maxJumlah} onChange={handleFilterChange} placeholder="Jumlah Max" className="p-2 border border-slate-300 rounded-lg w-full text-sm" />
                          <button onClick={resetFilters} className="p-2 border border-red-300 rounded-lg flex items-center gap-2 text-red-600 hover:bg-red-50 justify-center text-sm">
                              <X size={16} /> Reset
                          </button>
                      </div>
                  )}
                      ...
                      <tr>
                        {[{label: 'Batch ID', key: 'batch_id'}, {label: 'Nama Obat', key: 'nama_obat'}, {label: 'Tgl. Produksi', key: 'tanggal_produksi'}, {label: 'Jumlah', key: 'jumlah'}, {label: 'Status', key: 'status'}].map(header => (
                          <th key={header.key} className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">
                            <button onClick={() => requestSort(header.key)} className="flex items-center gap-1 hover:text-slate-800">
                               {header.label}
                               {sortConfig.key === header.key ? (
                                  sortConfig.direction === 'ascending' ? <ArrowUpDown size={14} className="text-emerald-600"/> : <ArrowUpDown size={14} className="text-emerald-600"/>
                               ) : (
                      ...
                      {produksiData.length > 0 ? (
                         produksiData.map((item) => (
                          <tr key={item.id} className="hover:bg-gray-50  transition-colors duration-150">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">{formatDate(item.tanggal_produksi)}</td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-emerald-700">{item.jumlah.toLocaleString('id-ID')} Pcs</td>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-4">
                              <button onClick={() => navigate(`/produsen/produksi/detail/${item.id}`)} className="text-blue-600 hover:text-blue-800" title="Lihat Detail">Detail</button>
                              <button onClick={() => navigate(`/produsen/produksi/edit/${item.id}`)} className="text-amber-600 hover:text-amber-800" title="Edit">Edit</button>
                              <button onClick={() => handleDelete(item.id)} className="text-red-600 hover:text-red-800" title="Hapus">Hapus</button>
                            </td>
                          </tr>
                         ))
                       ) : (
                         <tr>
                            <td colSpan="6" className="px-6 py-10 text-center text-slate-500">
                              ...
                              {searchTerm ? 'Tidak ada data produksi yang sesuai.' : 'Tidak ada jadwal produksi.'}
                            </td>
                          </tr>
                       )}
                    </tbody>
