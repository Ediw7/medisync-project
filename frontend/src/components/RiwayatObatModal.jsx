...
const RiwayatObatModal = ({ data, error, onClose, isLoading }) => {
  const getStatusInfo = (status) => {
    switch (status) {
      case 'DIPRODUKSI':
        return { icon: <CheckCircle className="text-blue-500" />, text: 'Diproduksi oleh' };
      case 'DIKIRIM_KE_PBF':
        return { icon: <CheckCircle className="text-purple-500" />, text: 'Dikirim ke PBF' };
      case 'DIKIRIM_KE_APOTEK':
        return { icon: <CheckCircle className="text-orange-500" />, text: 'Dikirim ke Apotek' };
      case 'TERJUAL_KE_KONSUMEN':
        return { icon: <CheckCircle className="text-green-500" />, text: 'Terjual kepada' };
      default:
        return { icon: <Clock className="text-gray-500" />, text: status };
    }
  };
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <DetailItem
                  ...
                  label="Produsen"
                  value={infoProduksi.pemilikSaatIni}
                />
                <DetailItem
                  icon={<Pill size={20} />}
                  label="Bentuk Sediaan & Dosis"
                  value={`${infoProduksi.bentukSediaan} - ${infoProduksi.dosis}`}
                />
                <DetailItem
                  icon={<Calendar size={20} />}
                  label="Tanggal Produksi"
                  value={new Date(infoProduksi.tanggalProduksi).toLocaleDateString('id-ID')}
                />
                <DetailItem
                  icon={<Calendar size={20} />}
                  label="Tanggal Kadaluarsa"
                  value={new Date(infoProduksi.tanggalKadaluarsa).toLocaleDateString('id-ID')}
                />
                <DetailItem
                  icon={<FileText size={20} />}
                  label="Nomor Izin Edar"
                  value={infoProduksi.nomorIzinEdar}
                />
                <DetailItem
                  icon={<Hash size={20} />}
                  label="Hash Sertifikat"
                  value={infoProduksi.hashDokumen.hasilUjiMutu.substring(0, 15) + '...'}
                />
              </div>
            </div>
