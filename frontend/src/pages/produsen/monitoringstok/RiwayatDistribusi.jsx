    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!distribusiResponse.ok) {
          ...
          throw new Error(
            `Gagal mengambil data distribusi: ${distribusiResponse.status} - ${errorText}`
          );
        }
        ...
        const distribusiBulanIniData = (distribusiResult.data || []).filter((item) => {
          ...
          try {
            ...
            return (
              !isNaN(tanggalData.getTime()) &&
              tanggalData.getMonth() === bulanIni &&
              tanggalData.getFullYear() === tahunIni
            );
          } catch (e) {
        ...
        const totalQuantityDistribusiBulanIni = distribusiBulanIniData.reduce(
          (sum, item) => sum + (item.jumlah_total_obat || 0),
          0
        );
        ...
        const mappedData = (distribusiResult.data || [])
          .map((item) => ({
            id: item.id,
                ...
                : item.status_blockchain === 'DIKIRIM_KE_PBF'
                  ...
                  : item.status === 'Dikirim'
                    ...
                    : item.status === 'Selesai'
                      ...
                      : item.status,
            assetId:
              item.detail_pesanan && item.detail_pesanan.length > 0
                ? item.detail_pesanan[0].blockchain_asset_id
                : null, // Get assetId for link
          }))
          .filter(
            (item) => item.status_pengiriman === 'Dikirim' || item.status_pengiriman === 'Diterima'
          );
    ...
    return distribusiData
      ...
      .filter((item) => {
        const searchableString =
          `${item.nomor_po || ''} ${item.nomor_surat_jalan || ''} ${item.tujuan || ''}`.toLowerCase();
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-lg mt-1">
                      Lacak pengiriman obat yang telah Anda distribusikan.
                    </p>
                  </div>
                </div>
                        ...
                        filteredData.map((item) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-emerald-700">
                              {item.jumlah_total_obat?.toLocaleString('id-ID') || '0'} Pcs
                            </td>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span
                                className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full border ${getStatusBadge(item.status_pengiriman)}`}
                              >
