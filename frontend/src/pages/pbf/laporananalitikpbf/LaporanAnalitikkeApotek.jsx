...
import { 
  ChevronDown, Loader2, AlertTriangle, BarChart as BarChartIcon
} from 'lucide-react';
...
const chartOptions = (title, yLabel = 'Jumlah') => ({
  ...
  scales: { 
    y: { beginAtZero: true, grid: { color: '#e2e8f0' }, ticks: { color: '#64748b' }, title: { display: true, text: yLabel } },
              ...
              <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-lg mt-1">Pantau distribusi, penjualan, dan performa apotek mitra.</p>
                  </div>
                </div>
              ...
              <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                  <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-96">
                    <Line data={lineData} options={chartOptions('Distribusi Obat ke Apotek', 'Box/Strip')} />
                  </div>
                  <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-96">
                    <Bar data={barPengiriman} options={chartOptions('Jumlah Pengiriman Bulanan', 'Pengiriman')} />
                  </div>
                </div>

                <div className="grid grid-cols-1 gap-6 mb-8">
                  <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-96">
                    <Bar data={barPenjualan} options={currencyChartOptions('Hasil Penjualan dengan Apotek')} />
                  </div>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                  <div className="lg:col-span-3 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-[500px]">
                    <Bar data={horizontalProduk} options={horizontalChartOptions('Top 10 Produk Terlaris')} />
                  </div>
                  <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-[500px]">
                    <Doughnut data={doughnutApotek} options={doughnutChartOptions('Top Apotek berdasarkan Pendapatan')} />
                  </div>
                </div>
              </>
            )}
