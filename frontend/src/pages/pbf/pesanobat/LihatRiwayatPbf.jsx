...
const LihatRiwayatPbf = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get(`http://localhost:5000/api/pbf/riwayat-distribusi/${assetId}`, { 
  ...
  const getTimestamp = (status) => {
      if (status === 'Diterima PBF' && dataDiterimaPbf) return formatTimestamp(dataDiterimaPbf.timestamp);
      if (status === 'Dikirim ke Apotek' && dataKirimApotek) return formatTimestamp(dataKirimApotek.timestamp);
              ...
              <div className="bg-gradient-to-r from-emerald-500 to-teal-600 px-6 py-5 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                 <div>
                  ...
                  <div className={`px-4 py-2 rounded-full border-2 text-sm font-semibold flex items-center gap-2 bg-white ${
                     currentStatus === 'Diterima Apotek' ? 'text-emerald-700 border-emerald-200' :
                     currentStatus === 'Dikirim ke Apotek' ? 'text-blue-700 border-blue-200' :
                     'text-amber-700 border-amber-200'
                 }`}>
                     {currentStatus === 'Diterima Apotek' ? <CheckCircle2 size={16} /> :
                      currentStatus === 'Dikirim ke Apotek' ? <Truck size={16} /> :
                      <Package size={16} />}
              ...
              <div className="p-8 py-12">
                ...
                <div className="flex items-start justify-center gap-0">
                  ...
                  <div className="relative flex items-center h-14 w-24">
                      <div className={`w-full h-1.5 rounded-full ${isDikirimApotekCompleted ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                  </div>
                  ...
                  <div className="relative flex items-center h-14 w-24">
                      <div className={`w-full h-1.5 rounded-full ${isDiterimaApotekCompleted ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                  </div>
