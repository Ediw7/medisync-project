...
const LacakPengembalianPbf = () => {
  ...
  const handleConfirmSelesai = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/api/pbf/pengembalian/${id}/selesaikan`,
        formData,
        {
          headers: {
  ...
  const alasanPengajuan =
    pesanan.alasan_pengembalian ||
    (pesanan.catatan_khusus
      ? pesanan.catatan_khusus.split('[PENGEMBALIAN DIAJUKAN]:')[1]?.split('[')[0]?.trim()
      : null) ||
    'Tidak ada alasan.';
  const alasanPenolakan =
    pesanan.alasan_penolakan_pengembalian ||
    (pesanan.catatan_khusus
  ...
  const steps = [
    {
      ...
      status: 'completed',
      icon: FileText,
      time: formatTimestamp(pesanan.updated_at),
    },
                ...
                <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
                  ...
                  <div className="p-6 space-y-4">
                    <p className="text-sm text-slate-600">
                      Apotek telah mengirimkan barang retur. Harap konfirmasi jika Anda sudah
                      menerimanya dalam kondisi baik.
                    </p>

                    {/* Form Upload Bukti PBF */}
                    <div>
                      <label className="block text-sm font-semibold text-slate-700 mb-2">
                        Unggah Bukti Penerimaan (PBF)*
                      </label>
                      <label
                        htmlFor="buktiFotoPbf"
                      ...
                      {buktiFotoPbf && (
                        <p className="text-xs text-slate-500 mt-2 truncate">
                          File terpilih: {buktiFotoPbf.name}
                        </p>
                      )}
                    </div>
