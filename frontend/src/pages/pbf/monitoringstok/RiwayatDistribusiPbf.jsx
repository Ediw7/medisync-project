    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!stokResponse.ok) {
          ...
          console.warn(
            `Gagal mengambil data statistik stok: ${stokResponse.status} - ${stokError}`
          );
        } else {
        ...
        if (!distribusiResponse.ok) {
          ...
          throw new Error(
            `Gagal mengambil data riwayat distribusi: ${distribusiResponse.status} - ${distError}`
          );
        }
    ...
    return distribusiData
      ...
      .filter((item) => {
        const searchableString =
          `${item.nomor_pesanan || ''} ${item.nomor_surat_jalan || ''} ${item.tujuan || ''}`.toLowerCase();
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-lg mt-1">
                      Lacak pengiriman obat yang telah Anda distribusikan ke Apotek.
                    </p>
                  </div>
                </div>
