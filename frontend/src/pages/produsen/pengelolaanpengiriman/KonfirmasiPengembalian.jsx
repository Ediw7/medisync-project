  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-lg shadow-xl p-8 max-w-md w-full">
        ...
        <div className="mt-6">
          <label
            htmlFor="alasan_penolakan_retur"
...
const KonfirmasiPengembalian = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `http://localhost:5000/api/produsen/pesanan-masuk/pengembalian/${cleanedId}`,
          {
        ...
        if (!response.data.success || !response.data.data) {
          throw new Error(
            response.data.message || 'Gagal mengambil data pengembalian atau format data salah'
          );
        }
        if (response.data.data.status !== 'Pengembalian Diajukan') {
          console.warn(
            `Status pesanan saat ini "${response.data.data.status || 'Tidak Diketahui'}".`
          );
        }
        const responseData = response.data.data;
        const totalHargaKeseluruhan = (responseData.detail_pesanan || []).reduce(
        ...
        toast.error(
          err.response?.data?.message || err.message || 'Gagal memuat data pengembalian.'
        );
        if (
  ...
  const handleAction = async (approve = true, alasan_penolakan = null) => {
    ...
    try {
      ...
      const errorMsg =
        err.response?.data?.message || err.message || `Gagal ${action} pengembalian.`;
      setError(errorMsg);
          ...
          <main className="flex-1 flex items-center justify-center p-6 pt-[72px]">
            <div className="bg-white p-8 rounded-2xl shadow-lg border border-slate-200 text-center max-w-md">
              ...
              <p className="text-slate-600 mb-6">
                Tidak dapat menemukan detail pengembalian untuk pesanan ini.
              </p>
              <button
                onClick={() => navigate('/produsen/pengelolaan-pengiriman/pengembalian')}
                className="flex items-center gap-2 px-4 py-2.5 border border-slate-300 text-slate-700 font-medium rounded-lg hover:bg-slate-100 transition mx-auto"
                ...
                {!canTakeAction && (
                  <div
                    className={`px-4 py-2 rounded-full border-2 text-sm font-semibold flex items-center gap-2 bg-white ${
                      info.status === 'Pengembalian Disetujui' || info.status === 'Dikembalikan'
                        ? 'text-emerald-700 border-emerald-200'
                        : info.status === 'Pengembalian Ditolak'
              ...
              <div className="p-8 border-b border-slate-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                      <span className="font-semibold text-slate-900 text-base">
                        {formatDate(info.tanggal_pengajuan_pengembalian || info.tanggal_pesanan)}
                      </span>
                    </div>
                  </div>
              ...
              <div className="p-6 flex flex-col sm:flex-row justify-end items-center gap-3 border-t border-slate-200">
                {canTakeAction ? (
                  ...
                  <p className="text-sm text-slate-600 mr-auto">
                    Tindakan telah diambil untuk pengajuan ini.
                  </p>
                )}
              </div>
              {/* --- AKHIR PERBAIKAN --- */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-lg shadow-xl p-8 max-w-md w-full text-center">
            ...
            <p className="text-gray-500 mt-2 text-sm">
              Anda akan menyetujui pengajuan pengembalian untuk pesanan ini. Pengiriman retur akan
              dijadwalkan. Dana akan dikembalikan setelah barang diterima.
            </p>
            <div className="mt-8 flex justify-center gap-4">
              <button
