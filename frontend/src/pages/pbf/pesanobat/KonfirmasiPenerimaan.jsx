      ...
      try {
        ...
        if (response.data.success && response.data.data) {
          if (response.data.data.pesanan?.status !== 'Dikirim') {
            setError(`Pesanan ini berstatus "${response.data.data.pesanan?.status || 'Tidak Diketahui'}". Hanya pesanan berstatus "Dikirim" yang dapat dikonfirmasi.`);
          }
        ...
        <main className="flex-1 overflow-auto pt-[72px] px-12 py-8">
          <div className="max-w-5xl mx-auto">
            <div className="print:hidden flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
              <button onClick={() => navigate('/pbf/pesan-obat')} className="inline-flex items-center text-emerald-600 hover:text-emerald-700 transition-colors text-sm font-medium">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-1">
                      ...
                      <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                        <span className="font-semibold text-slate-900 text-base">{info.nama_produsen || 'Produsen Tidak Diketahui'}</span>
                        <p className="text-sm text-slate-600">{info.alamat_produsen || 'Alamat tidak tersedia'}</p>
                      </div>
                    </div>
                    <div className="space-y-1">
              ...
              <footer className="flex flex-col sm:flex-row justify-end items-center gap-4 p-6 border-t border-slate-200 bg-slate-50 print:hidden">
                {info.status === 'Dikirim' && !error && (
                  <>
                    <Link to={`/pbf/pesanan/${id}/ajukan-pengembalian`} className="w-full sm:w-auto py-2.5 px-6 bg-red-100 text-red-700 font-medium rounded-lg hover:bg-red-200 border border-red-200 transition text-center text-sm">
                      Ajukan Pengembalian
                    </Link>
                    <button onClick={handleOpenConfirmModal} className="w-full sm:w-auto py-2.5 px-6 bg-emerald-600 text-white font-medium rounded-lg hover:bg-emerald-700 flex items-center justify-center gap-2 transition text-sm" disabled={isSubmitting}>
