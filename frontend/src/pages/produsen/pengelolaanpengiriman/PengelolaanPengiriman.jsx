...
import { Search, CalendarPlus, Loader2, Package, AlertTriangle, Truck, Plus } from 'lucide-react';
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-lg mt-1">
                      Kelola pesanan dan pengiriman ke PBF
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                  <div className="flex overflow-x-auto sm:overflow-visible w-full sm:w-auto">
                    <div className="flex space-x-2 bg-slate-100 p-1.5 rounded-lg">
                      ...
                      <NavItem to="/produsen/pengelolaan-pengiriman/perlu-dikirim">
                        Perlu dikirim
                      </NavItem>
                      ...
                      <NavItem to="/produsen/pengelolaan-pengiriman/pengembalian">
                        Pengembalian
                      </NavItem>
                    </div>
                  </div>
                    ...
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        <th
                          ...
                          onClick={() => requestSort('nama_pbf')}
                        >
                          PBF{' '}
                          {sortConfig.key === 'nama_pbf' &&
                        ...
                        <th
                          className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100"
                          onClick={() => requestSort('total_harga')}
                        >
                          Total Harga{' '}
                          ...
                          >
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                              {item.status === 'Perlu Dikirim' && (
                                <Link
                                  to={`/produsen/pengelolaan-pengiriman/atur-pengiriman/${item.id}`}
                              ...
                              {item.status === 'Dikirim' && (
                                <Link
                                  to={`/produsen/pengelolaan-pengiriman/lihat-status/${item.id}`}
                              ...
                              {item.status === 'Pembatalan Diajukan' && (
                                <Link
                                  to={`/produsen/pengelolaan-pengiriman/konfirmasi-pembatalan/${item.id}`}
                              ...
                              {item.status === 'Dibatalkan' && (
                                <Link
                                  to={`/produsen/pengelolaan-pengiriman/riwayat-pembatalan/${item.id}`}
                              ...
                              {item.status === 'Pengembalian Diajukan' && (
                                <Link
                                  to={`/produsen/pengelolaan-pengiriman/konfirmasi-pengembalian/${item.id}`}
                              ...
                              {(item.status === 'Dikembalikan' ||
                                item.status === 'Pengembalian Disetujui' ||
