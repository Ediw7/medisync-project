...
const DibatalkanPbf = () => {
  ...
  const statusFilter = 'Dibatalkan'; // Filter di-hardcode
  ...
  if (isLoading && pesananList.length === 0) {
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50">
        ...
        <p className="mt-4 text-slate-700 font-medium">Memuat Pesanan Dibatalkan...</p>
      </div>
    );
  }
          ...
          <div className="max-w-7xl mx-auto">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-lg mt-1">
                      Kelola pesanan masuk yang membutuhkan tindakan
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => navigate('/pbf/pengelolaan-pesanan/pengiriman-massal')}
                  className="bg-emerald-600 text-white font-medium py-2.5 px-5 rounded-lg hover:bg-emerald-700 transition flex items-center gap-2 shadow-sm whitespace-nowrap w-full sm:w-auto justify-center relative z-20"
                >
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden relative z-10">
              <div className="p-4 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4">
                {/* --- NAVIGASI TAB (DIMASUKKAN KEMBALI) --- */}
                  ...
                  <table className="min-w-full">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        ...
                        >
                          <div className="flex items-center gap-1">
                            Tgl. Pesan {getSortIndicator('tanggal_pesanan')}
                          </div>
                        </th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">
                          Status
                        ...
                        filteredAndSortedData.map((order) => (
                          <tr key={order.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                              <Link
                                to={`/pbf/pengelolaan-pesanan/surat/${order.id}`}
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <span
                                className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusClass(order.status)}`}
                              >
                                {order.status}
                              </span>
                            </td>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                              {order.status === 'Pembatalan Diajukan' && (
                                <Link
                                  to={`/pbf/pengelolaan-pesanan/konfirmasi-pembatalan/${order.id}`}
                              ...
                              {order.status === 'Dibatalkan' && (
                                <Link
                                  to={`/pbf/pengelolaan-pesanan/riwayat-pembatalan/${order.id}`}
