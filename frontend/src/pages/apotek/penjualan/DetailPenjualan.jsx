...
const InfoItem = ({ icon: Icon, label, value, mono = false, highlight = false }) => (
  <div className="space-y-1">
    ...
    <p
      className={`font-semibold text-base ${mono ? 'font-mono' : ''} ${highlight ? 'text-emerald-700' : 'text-slate-700'}`}
    >
      {value || '-'}
    </p>
  </div>
    ...
    const fetchDetail = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `http://localhost:5000/api/apotek/penjualan/riwayat/${id}`,
          {
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-lg mt-1">
                      ...
                      <span className="font-medium text-slate-700 font-mono">
                        #{String(penjualan.id).padStart(6, '0')}
                      </span>
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <InfoItem
                    ...
                    value={formatDate(penjualan.tanggal_penjualan)}
                  />
                  <InfoItem
                    icon={DollarSign}
                    label="Total Transaksi"
                    value={`Rp ${penjualan.total_harga.toLocaleString('id-ID')}`}
                    highlight
                  />
                </div>
              </div>
                ...
                <div className="p-6 space-y-4">
                  {detail.map((item) => (
                    <div
                      ...
                      className="flex flex-col sm:flex-row items-center gap-4 p-4 bg-white rounded-lg border border-slate-200"
                    >
                      ...
                      <div className="flex-1 text-center sm:text-left">
                        <p className="font-semibold text-slate-800">
                          {item.nama_obat || 'Nama Obat Tidak Tercatat'}
                        </p>
                        <p className="text-sm text-slate-600">
                          Jumlah Terjual:{' '}
                          <span className="font-medium text-emerald-700">{item.jumlah_jual}</span>
                        </p>
                        <p className="text-xs text-slate-500 font-mono mt-1">
                          Batch ID: {item.id_aset_blockchain.slice(-12)}
                        </p>
                        <a
