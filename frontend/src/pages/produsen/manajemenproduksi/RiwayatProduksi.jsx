              ...
              <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-lg mt-1">Daftar produksi yang telah dicatat di blockchain.</p>
                  </div>
                </div>
              </div>
            </div>
                      ...
                      {riwayatProduksi.length > 0 ? (
                         riwayatProduksi.map((item) => (
                          <tr key={item.id} className="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                              {item.qr_code_url ? (
                                <img src={item.qr_code_url} alt="QR Code" className="w-16 h-16 rounded-md border border-slate-200 p-1 bg-white" />
                              ) : (
                                <span className="text-xs text-slate-400 italic">T/A</span>
                              )}
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                              <button onClick={() => navigate(`/produsen/produksi/detail/${item.id}`)} className="text-blue-600 hover:text-blue-800 font-semibold" title="Lihat Detail">Detail</button>
                            </td>
                          </tr>
                         ))
                       ) : (
                         <tr>
                            <td colSpan="7" className="px-6 py-10 text-center text-slate-500">
                              ...
                              {searchTerm ? 'Tidak ada riwayat yang sesuai pencarian.' : 'Belum ada produksi yang dicatat di blockchain.'}
                            </td>
                          </tr>
                       )}
                    </tbody>
