    ...
    >
      ...
      >
        ...
        <p className="text-gray-500 mt-2 text-sm">
          Anda akan menyetujui pengajuan pengembalian. Apotek akan diminta mengirimkan barang
          kembali.
        </p>
        <div className="mt-8 flex justify-center gap-4">
          <button
...
const KonfirmasiPengembalianPbf = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.data.data.pesanan.status !== 'Pengembalian Diajukan') {
          console.warn(
            `Status pesanan saat ini "${response.data.data.pesanan.status || 'Tidak Diketahui'}".`
          );
        }
  ...
  const alasanPengajuan =
    ...
    (info.catatan_khusus
      ? info.catatan_khusus.split('[PENGEMBALIAN DIAJUKAN]:')[1]?.trim()
      : '-') ||
    'Tidak ada alasan.';
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 flex items-center justify-between">
                      <span className="font-semibold text-slate-900 text-base">
                        {info.nomor_pesanan || `Pesanan #${String(info.id).padStart(6, '0')}`}
                      </span>
                      <Link
