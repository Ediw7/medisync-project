...
import { 
  ChevronDown, Search, Calendar, Loader2, AlertTriangle, 
  BarChart as BarChartIcon, DollarSign, Archive, Package, 
...
const LaporanAnalitikPbf = () => {
  ...
  const chartOptions = (title) => ({
    ...
    plugins: { 
      ...
      title: { display: true, text: title, font: { size: 16, weight: '600' }, padding: { bottom: 20 }, color: '#1e293b' }
    },
    scales: { 
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const [barRes, pieRes, tableRes, kpiRes] = await Promise.all([
          axios.get(`http://localhost:5000/api/pbf/laporan/pemesanan-bulanan-produsen`, { headers: { Authorization: `Bearer ${token}` } }),
          axios.get('http://localhost:5000/api/pbf/laporan/transaksi-per-produsen', { headers: { Authorization: `Bearer ${token}` } }),
          axios.get('http://localhost:5000/api/pbf/laporan/riwayat-produsen', { headers: { Authorization: `Bearer ${token}` } }),
          axios.get('http://localhost:5000/api/pbf/laporan/kpi-data', { headers: { Authorization: `Bearer ${token}` } })
        ]);
          ...
          <div className="max-w-7xl mx-auto">
              ...
              <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-lg mt-1">Overview performa bisnis dan distribusi ke apotek.</p>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
              {isLoading || !kpiData ? (
                Array(3).fill().map((_, i) => (
                  <div key={i} className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 h-28 animate-pulse">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <h2 className="text-lg font-semibold text-slate-800">Riwayat Transaksi (10 Terbaru)</h2>
                <div className="flex gap-3 w-full sm:w-auto">
                  <div className="relative flex-1 sm:flex-none">
                    <Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                    <input type="text" placeholder="Cari ID Pesanan..." className="pl-10 pr-4 py-2 border border-slate-300 rounded-lg text-sm w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                  </div>
                  <div className="relative">
                    <Calendar size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                    <input type="text" placeholder="Pilih Tanggal" className="pl-10 pr-4 py-2 border border-slate-300 rounded-lg text-sm w-full sm:w-40 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                  </div>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">ID Pesanan</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Tanggal</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Nama Produsen</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Jumlah Item</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Status</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {isLoading ? (
                      <tr><td colSpan="5" className="py-12"><ChartPlaceholder message="Memuat riwayat..." type="loading" /></td></tr>
                    ) : riwayatData.length > 0 ? (
                      riwayatData.map(item => (
                        <tr key={item.id} className="hover:bg-emerald-50 transition">
                          <td className="px-6 py-4 text-sm font-semibold text-emerald-700">#{String(item.id).padStart(6, '0')}</td>
                          <td className="px-6 py-4 text-sm text-slate-600">{formatDate(item.tanggal_pesanan)}</td>
                          <td className="px-6 py-4 text-sm font-medium text-slate-900">{item.nama_produsen}</td>
                      ...
                      <tr><td colSpan="5" className="py-12"><ChartPlaceholder message="Tidak ada riwayat." type="empty" /></td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              <div className="text-center mt-6">
                <Link to="/pbf/laporan-riwayat" className="text-emerald-600 font-semibold text-sm hover:underline inline-flex items-center gap-1">
