...
const ProdusenToPbfTimeline = ({ onChain, offChain, onShowBukti }) => {
  ...
  if (isPengembalian) {
    return (
      <div className="flex items-start justify-center gap-0">
        ...
        <StatusStep icon={XCircle} label={offChain.status} isCompleted={true} isCurrent={true}>
  ...
  return (
    <div className="flex items-start justify-center gap-0">
      ...
      <div className="relative flex items-center h-14 w-24">
        <div
          className={`w-full h-1.5 rounded-full ${isDikirim ? 'bg-emerald-500' : 'bg-slate-300'}`}
        />
      </div>
      <StatusStep
        icon={Truck}
      ...
      <div className="relative flex items-center h-14 w-24">
        <div
          className={`w-full h-1.5 rounded-full ${isDiterima ? 'bg-emerald-500' : 'bg-slate-300'}`}
        />
      </div>
      <StatusStep
        icon={Home}
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `http://localhost:5000/api/pbf/pesanan/riwayat/${assetId}`,
          {
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              <div className="bg-gradient-to-r from-emerald-500 to-teal-600 px-6 py-5 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                ...
                <div
                  className={`px-4 py-2 rounded-full border-2 text-sm font-semibold flex items-center gap-2 bg-white ${
                    currentStatus === 'Selesai' || currentStatus === 'Pengembalian Selesai'
                    ...
                    <tfoot className="bg-slate-50 font-semibold">
                      <tr>
                        ...
                        <td className="px-4 py-3 text-right text-xl text-emerald-700">
                          Rp{' '}
                          {Number(offChain.total_harga || 0).toLocaleString('id-ID', {
                            minimumFractionDigits: 2,
                          })}
                        </td>
                      </tr>
              ...
              <div className="p-8 py-12">
                ...
                <ProdusenToPbfTimeline
                  onChain={onChain}
                  offChain={offChain}
                  onShowBukti={() => setIsModalOpen(true)}
                />
              </div>
              ...
              <div className="px-8 pb-8">
                <div className="bg-blue-50 border border-blue-200 text-blue-800 rounded-xl p-4 text-sm flex items-start gap-3">
                  ...
                  <span>
                    Status pengiriman ini diverifikasi berdasarkan data blockchain untuk
