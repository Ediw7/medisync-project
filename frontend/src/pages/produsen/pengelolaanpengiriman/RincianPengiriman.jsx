...
const generateSuratJalanNumber = (orderId) => {
  ...
  if (!nomorIzin) {
    console.error("Nomor Izin produsen tidak ditemukan di localStorage.");
...
const RincianPengiriman = () => {
  ...
  useEffect(() => {
    if (pesananFromState) {
        console.log("Menggunakan data pesanan dari location.state");
        ...
        if (nomorSuratJalan === 'ERROR-NO-IZIN') {
           setError("Nomor Izin produsen tidak ditemukan. Silakan logout dan login kembali untuk membuat Surat Jalan.");
           toast.error("Nomor Izin produsen tidak ditemukan.");
        }
    } else {
        console.log("Memuat data pesanan dari API...");
        const fetchData = async () => {
          ...
          try {
            ...
            setAlamatTujuan(response.data.data.pesanan.alamat_pbf || 'Alamat tidak tersedia');

            if (!tanggalPengiriman) setTanggalPengiriman(new Date().toISOString().split('T')[0]);

          } catch (error) {
            const errorMsg = error.response?.data?.message || error.message || 'Gagal memuat data pesanan.';
            setError(errorMsg);
            toast.error(errorMsg);
            if ((errorMsg.includes('401') || errorMsg.includes('403') || errorMsg.includes('login')) && token) {
  ...
  }, [id, navigate, pesananFromState, tanggalPengiriman, nomorSuratJalan]);
  ...
  const handleSubmit = (e) => {
    ...
    if (nomorSuratJalan === 'ERROR-NO-IZIN') {
        const msg = 'Nomor Izin Produsen tidak ditemukan. Tidak dapat membuat Surat Jalan.';
        setError(msg);
        toast.error(msg);
        return;
    }
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success('Status pesanan diperbarui, stok diperbarui, dan data siap dicatat.', { id: toastId });
