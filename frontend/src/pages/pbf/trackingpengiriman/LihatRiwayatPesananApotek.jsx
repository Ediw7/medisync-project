...
import SidebarPbf from '../../../components/SidebarPbf'; // DIUBAH: Menggunakan Sidebar PBF
import NavbarPbf from '../../../components/NavbarPbf';   // DIUBAH: Menggunakan Navbar PBF
import { ArrowLeft, CheckCircle, Package, Truck, Loader2, X, ClipboardCopy } from 'lucide-react';
import axios from 'axios'; // DIUBAH: Menggunakan axios untuk konsistensi
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-white p-4 rounded-lg shadow-2xl relative w-full max-w-lg" onClick={e => e.stopPropagation()}>
                ...
                {fullImageUrl ? <img src={fullImageUrl} alt="Bukti Penerimaan Barang" className="w-full h-auto max-h-[70vh] object-contain rounded"/> : <p className="text-center text-slate-500">Gambar tidak tersedia.</p>}
            </div>
        </div>
    );
};

// Komponen StatusStep tidak perlu diubah
...
const LihatRiwayatPesananApotek = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (!token) {
                    navigate('/login/pbf'); // DIUBAH: Arahkan ke login PBF jika tidak ada token
                    return;
                }
                // DIUBAH: Panggil endpoint riwayat yang sesuai di sisi PBF
                ...
                // Menangani error dari axios atau dari response.data.success = false
    ...
    // DIUBAH: Menampilkan pesan error yang lebih informatif
    if (error) return <div className="p-6 text-center text-red-500">Error: {error}</div>;
    if (!riwayatData) return <div className="p-6 text-center text-gray-500">Data riwayat tidak ditemukan.</div>;
    ...
    return (
        <div className="flex min-h-screen bg-gray-50">
            {/* DIUBAH: Menggunakan komponen PBF */}
            <SidebarPbf isCollapsed={isCollapsed} setIsCollapsed={setIsCollapsed} />
            <div className={`flex-1 flex flex-col transition-all duration-300 ${isCollapsed ? 'ml-16' : 'ml-64'}`}>
                <NavbarPbf onLogout={() => { localStorage.clear(); navigate('/'); }} />
                <main className="pt-16 p-6">
                    <div className="max-w-4xl mx-auto">
                        {/* DIUBAH: Tombol kembali mengarah ke halaman pengelolaan pesanan PBF */}
                        <button onClick={() => navigate('/pbf/pengelolaan-pesanan')} className="flex items-center gap-2 text-gray-600 hover:text-gray-900 font-semibold mb-6">
                            <ArrowLeft size={18} /> Kembali
                        </button>
                        <div className="bg-white p-8 rounded-lg shadow-lg border">
                            <header className="mb-8 text-center">
                                {/* DIUBAH: Judul disesuaikan untuk PBF */}
                                <h1 className="text-2xl font-bold text-gray-800">Riwayat Pengiriman ke Apotek</h1>
                                <p className="text-gray-500 mt-1">Lacak jejak pengiriman dari PBF ke Apotek</p>
                            </header>
                             <section className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12 text-sm">
                                <div>
                                    <p className="text-gray-500">No Resi</p>
                                    <div className="flex items-center gap-2 mt-1">
                                        <p className="font-semibold text-lg">{offChain.nomor_resi || '-'}</p>
                                ...
                                <div><p className="text-gray-500">Tujuan (Apotek)</p><p className="font-semibold text-lg mt-1">{offChain.nama_apotek}</p></div>
                            ...
                            <section className="flex justify-center py-6">
                                <StatusStep icon={<Package size={24}/>} label="Dipesan" timestamp={formatDate(offChain.tanggal_pesanan)} isCompleted={true} />
                                <StatusStep icon={<Truck size={24}/>} label="Dikirim" timestamp={dataKirim ? formatDate(offChain.tanggal_pengiriman) : null} isCompleted={!!dataKirim} />
                                <StatusStep icon={<CheckCircle size={24}/>} label="Diterima" timestamp={dataTerima ? formatTimestamp(dataTerima.timestamp) : null} isCompleted={!!dataTerima} isLast={true}>
                                    {!!dataTerima && offChain.bukti_foto && (
...
export default LihatRiwayatPesananApotek;