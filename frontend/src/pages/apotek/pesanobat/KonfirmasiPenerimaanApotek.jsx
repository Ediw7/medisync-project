...
const ConfirmationModal = ({
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex justify-center items-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in-0"
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <div className="text-center pt-5">
          ...
          <p className="text-slate-600 mb-5 text-sm">
            Anda akan mengkonfirmasi penerimaan pesanan:
            <br />
...
const StatusLine = ({ isCompleted }) => (
  <div className="relative flex items-center h-14 w-16 md:w-24">
    <div
      className={`w-full h-1.5 rounded-full ${isCompleted ? 'bg-emerald-500' : 'bg-slate-300'}`}
    />
  </div>
);
// --- AKHIR STATUS STEP ---
      ...
      try {
        ...
        if (response.data.success && response.data.data) {
          if (response.data.data.pesanan?.status !== 'Dikirim') {
            setError(
              `Pesanan ini berstatus "${response.data.data.pesanan?.status || 'Tidak Diketahui'}". Hanya pesanan "Dikirim" yang dapat dikonfirmasi.`
            );
            toast.error(`Pesanan ini berstatus "${response.data.data.pesanan?.status}".`, {
              duration: 4000,
            });
          }
                          ...
                          detail.map((item, index) => (
                            <tr key={item.id || index} className="hover:bg-slate-50">
                              ...
                              <td className="px-4 py-3 text-center font-medium text-emerald-700">
                                {(item.jumlah_pesanan || 0).toLocaleString('id-ID')}{' '}
                                {item.satuan || 'Box'}
                              </td>
              ...
              <div className="space-y-6">
                  ...
                  <div className="p-6">
                    ...
                    {info.status === 'Dikirim' && !error && (
                      <>
                        ...
                        <Link
                          to={`/apotek/pesanan/${id}/ajukan-pengembalian`}
                          className="mt-3 w-full py-3 px-6 bg-red-100 text-red-700 font-medium rounded-lg hover:bg-red-200 border border-red-200 transition text-center text-sm flex items-center justify-center gap-2"
                        >
                ...
                <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
                  ...
                  <p className="text-sm text-slate-600">
                    Mengkonfirmasi penerimaan akan memperbarui status aset di blockchain dan
                    memindahkan stok ke inventaris Anda.
                  </p>
                </div>
              </div>
            </div>
