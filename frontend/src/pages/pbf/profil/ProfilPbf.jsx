...
const ProfileDetails = ({ profile, onProfileUpdate }) => {
  ...
  useEffect(() => {
    // Selalu update form jika data dari parent berubah
    setFormData({ ...profile });
  }, [profile]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        onProfileUpdate(response.data.data); // Update data di parent
        toast.success('Profil berhasil diperbarui!');
        setIsEditing(false);
        // Perbarui data di localStorage untuk Navbar
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-emerald-50 to-emerald-100 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
          <h2 className="text-lg font-semibold text-emerald-900 flex items-center gap-2">
            <Building size={20} /> Informasi Perusahaan
          </h2>
          <button
            type="button"
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          {isEditing ? (
            ...
            <>
              ...
              <InfoItem label="Nama Resmi PBF" value={formData.nama_resmi} icon={Building} />
              <InfoItem label="Kontak Telepon" value={formData.kontak_telepon} icon={Phone} />
              <InfoItem label="Nomor Izin (SIUP)" value={formData.nomor_izin} icon={FileText} />
              <InfoItem label="Nomor SIA/SIKA" value={formData.nomor_sia_sika} icon={FileText} />
...
// --- Komponen Ganti Password (Sudah Benar) ---
  ...
  <div>
    ...
    <div className="relative">
      ...
      <input
        ...
        name={name}
        type={type}
        value={value || ''} // <-- Tambahkan '|| ''' untuk mencegah error 'uncontrolled component'
        onChange={onChange}
        disabled={disabled}
        required={required}
