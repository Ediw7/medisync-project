...
const PengirimanMassal = () => {
  ...
  if (isLoading && pesananData.length === 0) {
    // Hanya tampil jika data awal belum ada
    return (
  ...
  if (error && pesananData.length === 0) {
    // Hanya tampil jika data awal gagal load
    return (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-lg mt-1">
                      Atur pengiriman untuk beberapa pesanan sekaligus
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 border-b border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                  <span className="text-sm text-slate-500 font-medium">
                    {selectedPesanan.length} dipilih
                  </span>
                  <button
                        ...
                        <th className="px-4 py-3 text-left">
                          <input
                            ...
                            checked={
                              ...
                              selectedPesanan.length === processedData.length
                            }
                    ...
                    <tbody className="bg-white divide-y divide-slate-100">
                      {processedData.length > 0 ? (
                        processedData.map((item) => (
                          <tr
                            key={item.id}
                            className={`hover:bg-gray-50 transition-colors duration-150 ${selectedPesanan.includes(item.id) ? 'bg-emerald-50' : ''}`}
                          >
                            ...
                            <td className="px-4 py-4 whitespace-nowrap text-sm text-slate-600">
                              {(item.detail_pesanan || []).map((detail) => (
                                <p
                                  ...
                                  title={`${detail.nama_obat} (Batch: ${detail.batch_id || 'N/A'})`}
                                >
                                  {detail.nama_obat}{' '}
                                  <span className="text-xs text-slate-400">
                                    (Batch: {detail.batch_id || 'N/A'})
                                  </span>
                                </p>
                              ))}
