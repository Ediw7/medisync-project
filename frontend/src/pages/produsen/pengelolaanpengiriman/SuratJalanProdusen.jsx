...
const SuratJalanProdusen = () => {
  ...
  const [data, setData] = useState(null); // Satu state untuk semua data
            ...
            <div ref={contentRef} className="bg-white p-8 border border-gray-300 shadow-md print:border-0 print:shadow-none">
              <header className="flex justify-between items-start mb-8 pb-4 border-b-2 border-black">
                  <div className="flex flex-col">
                      <h1 className="text-2xl font-bold text-gray-800">{info.nama_produsen}</h1>
                      <p className="text-xs text-gray-600">{info.alamat_produsen}</p>
                  </div>
                  <div className="flex flex-col text-right">
                      ...
                      <p className="text-sm text-gray-500">Tanggal: {new Date(info.tanggal_pengiriman).toLocaleDateString('id-ID')}</p>
                  </div>
              </header>
              ...
              <section className="grid grid-cols-2 gap-8 mb-8 text-sm">
                  <div>
                      ...
                      <p>{info.nama_produsen}</p>
                      <p>{info.alamat_produsen}</p>
                  </div>
                  <div>
                      <h3 className="font-bold text-gray-800 mb-2">PENERIMA</h3>
                      <p className="font-semibold">{info.nama_pbf}</p>
                      <p>{info.alamat_pbf}</p>
