    ...
    >
      ...
      >
        ...
        <div className="mt-4 text-center">
            ...
            <p className="text-slate-600 mb-6">Penjualan telah berhasil dicatat. Berikan QR code ini kepada pelanggan untuk melacak riwayat obat.</p>
        </div>

        {/* Daftar QR Code */}
        <div className="space-y-4 max-h-[40vh] overflow-y-auto p-4 bg-slate-50 rounded-lg border">
          {soldAssetIds.map((asset, index) => (
            <div key={index} className="flex flex-col sm:flex-row items-center gap-4 p-4 bg-white rounded-lg border">
              ...
              <div className="flex-1 text-center sm:text-left">
                ...
                <p className="text-xs text-slate-500 font-mono">Batch ID: {asset.id_aset_blockchain.slice(-6)}</p>
...
const Penjualan = () => {
  ...
  const availableStock = useMemo(() => {
    return stokApotek
      .map(stokItem => {
        const itemInCart = keranjang.find(cartItem => cartItem.id_aset_blockchain === stokItem.id);
  ...
  const handleAddItem = () => {
    ...
    const existingItemIndex = keranjang.findIndex(item => item.id_aset_blockchain === selectedObat.id);
  ...
  const handleProsesPembayaran = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/apotek/penjualan/proses', payload, {
        headers: { Authorization: `Bearer ${token}` }
      });
                ...
                <div className="p-6 border-b border-slate-200">
                  <div className="flex flex-col md:flex-row gap-4">
                    ...
                    <div className="relative flex-grow">
                       <label className="block text-sm font-medium text-slate-700 mb-2">Cari Obat (Nama atau Batch)</label>
                      <Search className="absolute left-3 top-[calc(50%+8px)] -translate-y-1/2 h-5 w-5 text-slate-400" />
                      <input 
                        type="text" 
                        placeholder={isStokLoading ? "Memuat stok..." : "Cari nama obat atau batch ID..."}
                      ...
                      {searchTerm && searchResults.length > 0 && (
                        <div className="absolute z-20 w-full mt-1 bg-white border border-slate-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                          {searchResults.map(item => (
                            ...
                            >
                              <p className="font-medium text-sm text-slate-800">{item.nama_obat} <span className="text-xs text-slate-500">({item.dosis})</span></p>
                              <p className="text-xs text-slate-600 font-mono">Batch: {item.id.slice(-6)} (Stok: {item.stok_saat_ini})</p>
                            </div>
                          ))}
                        </div>
                      )}
                       {searchTerm && searchResults.length === 0 && !isStokLoading && (
                           <div className="absolute z-10 w-full mt-1 bg-white border border-slate-300 rounded-lg shadow-lg p-4">
                                <p className="text-sm text-slate-500 text-center">Obat tidak ditemukan.</p>
                           </div>
                       )}
                    </div>
                    {/* --- AKHIR AUTOCOMPLETE --- */}
                    ...
                    <div className="flex gap-3">
                       ...
                       <div className="flex-shrink-0">
                         <label className="block text-sm font-medium text-slate-700 mb-2 opacity-0">Aksi</label>
                         <button 
                  ...
                  <table className="min-w-full">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {keranjang.length > 0 ? keranjang.map(item => (
                        <tr key={item.id_aset_blockchain} className="hover:bg-slate-50 transition-colors">
                          <td className="px-6 py-4">
                            <p className="text-sm font-semibold text-slate-900">{item.nama_obat}</p>
                        ...
                        <tr>
                          <td colSpan="5" className="text-center py-12">
                            ...
                            <p className="text-xs text-slate-400 mt-1">Cari obat di atas untuk menambahkannya ke keranjang.</p>
                          </td>
                        </tr>
                      )}
                    </tbody>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 h-fit sticky top-24">
                ...
                <div className="space-y-3">
                  <button 
                    ...
                    disabled={isSubmitting || keranjang.length === 0 || (Number(nominalBayar) || 0) < total}
